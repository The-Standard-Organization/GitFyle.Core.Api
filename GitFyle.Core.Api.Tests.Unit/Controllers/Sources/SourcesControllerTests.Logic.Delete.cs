...
namespace GitFyle.Core.Api.Tests.Unit.Controllers.Sources
{
    public partial class SourcesControllerTests
    {
        ...
        public async Task ShouldRemoveRecordOnDeleteByIdsAsync()
        {
            ...
            sourceServiceMock
                .Setup(service => service.RemoveSourceByIdAsync(It.IsAny<Guid>()))
                    .ReturnsAsync(storageSource);

            // when
            ActionResult<Source> actualActionResult = await sourcesController.DeleteSourceByIdAsync(inputId);

            // then
            actualActionResult.ShouldBeEquivalentTo(expectedActionResult);

            sourceServiceMock
                .Verify(service => service.RemoveSourceByIdAsync(It.IsAny<Guid>()),
                    Times.Once);
